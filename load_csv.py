#!/usr/bin/env python3
"""
Load OHLC data from CSV into the Lipstick Trading System V5 database.

Usage:
    python load_csv.py [csv_filename] [symbol]

Example:
    python load_csv.py ES_12192025.csv ES
    python load_csv.py NQ_12192025.csv NQ
"""
...
CSV_FOLDER = 'Raw CSV Data'

def load_csv_to_db(csv_filename, symbol):
    """
    Load OHLC data from CSV file into database.

    Args:
        csv_filename: Name of CSV file in Raw CSV Data folder
    ...
    print(f"\nLoading {csv_filename} for symbol: {symbol}")
    print("=" * 80)
...
def verify_data(symbol):
    ...
    cursor.execute("SELECT COUNT(*) FROM ohlc_1m WHERE symbol = ?", (symbol,))
    count = cursor.fetchone()[0]
    print(f"Records in database for {symbol}: {count}")
...
def main():
    """Main entry point."""
    if len(sys.argv) != 3:
        print("Usage: python load_csv.py [csv_filename] [symbol]")
        print("\nExample:")
        print("  python load_csv.py ES_12192025.csv ES")
        print("  python load_csv.py NQ_12192025.csv NQ")
        sys.exit(1)
    ...
    print(f"\n[OK] Successfully loaded {stats['inserted']} new records for {symbol}")
