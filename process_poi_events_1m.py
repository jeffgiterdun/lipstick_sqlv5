"""
POI Event Processing Script for Daily, Weekly, and Monthly Sessions (1M Database)

This script processes Point of Interest (POI) touches for Daily, Weekly, and Monthly sessions
in the ohlc_data.db database. It:

1. Scans 1M OHLC data for touches of PoC, RPP, and TO levels
...
6. Handles Minor session expiration (36 hours after TO)

CRITICAL BUG FIX APPLIED:
- Skips the TO candle itself when scanning for POI touches (line 549)
- The TO candle is the reference point, not a "touch"
...
Usage:
    ...
    python process_poi_events_1m.py --incremental
"""
...
def get_candles_after_time(
    ...
    """
    Get all 1M candles for a symbol after start_time.
...
def get_or_create_poi_event(
    ...
    Args:
        ...
        session_type: 'Major', 'Minor', 'Weekly', or 'Monthly'
    ...
    if existing:
        # Check if this symbol already has a time recorded
        event_id = existing['id']
        es_time = existing['es_event_time']
        nq_time = existing['nq_event_time']

        # If this symbol already has a time, this is a SECOND touch (e.g., second break)
        # Create a NEW event instead of updating the existing one
        if symbol == 'ES' and es_time is not None:
            # ES already has a time - create new event for second touch
            existing = None
        elif symbol == 'NQ' and nq_time is not None:
            # NQ already has a time - create new event for second touch
            existing = None

    if existing:
        # Update existing event (first touch for this symbol)
        event_id = existing['id']
        es_time = existing['es_event_time']
        nq_time = existing['nq_event_time']
...
def is_session_expired(session: Dict, current_time: str) -> bool:
    """
    Check if a session has expired.

    Minor sessions expire 36 hours after TO time.
    Major, Weekly, and Monthly sessions never expire.
...
def process_session(
    ...
    for symbol in ['ES', 'NQ']:
        ...
        for candle in candles:
            ...
            # CRITICAL BUG FIX: Skip the TO candle itself!
            # The TO candle is the definition/reference point, not a touch
            if candle_time == to_time:
                continue

            # Check if this candle is beyond expiration (Minor sessions only)
            # For hindsight system: Check candle timestamp, not upload time
            if is_session_expired(current_symbol_session, candle_time):
                continue
            ...
            for poi_type, poi_value in [('PoC', poc), ('RPP', rpp), ('TO', to)]:
                if detect_touch(candle, poi_type, poi_value):
                    ...
                    if status_changed:
                        ...
                        # Break after first successful state change in this candle
                        # (Don't process multiple POI types if one already triggered a state change)
                        break
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Process POI events for Daily, Weekly, and Monthly sessions (1M database)',
...
Examples:
  ...
  python process_poi_events_1m.py --incremental
    ...
    print("=" * 80)
    print("POI Event Processing - Daily, Weekly, and Monthly Sessions (1M)")
    print("=" * 80)
    print()
    ...
    try:
        # Get latest data time from ohlc_1m table
